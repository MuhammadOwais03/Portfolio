...
interface ProjectsProps {
    params: {
        slug: string; // The dynamic part of the URL
    };
}
...
const Projects: React.FC<ProjectsProps> = ({ params }) => {
    ...
    useEffect(() => {
        const fetchProjectNames = async () => {
            try {
                ...
                `);
                setAllProjectsName(data || []);  // Handle the case when no project is found
            } catch (error) {
                console.error("Error fetching project names:", error);
            }
        };

        fetchProjectNames(); // Calling the fetchProjectNames function
    ...
    useEffect(() => {
        const fetchProject = async () => {
            ...
            setProject(data.length > 0 ? data[0] : null); // Handle the case when no project is found
        };

        fetchProject();
    }, [params.slug]); // This effect runs whenever params.slug changes
    ...
    if (!project) {
        return <Loading/>; // Add a loading state for better UX
    }
        ...
        <>
            ...
            <div id="projects">
                ...
                <div className="projects-dropdown">
                    <i className={`bx ${isMenuOpen ? "bx-x" : "bx-menu"}`} onClick={toggleMenu} style={styles.icon}></i>
                    <div className={`dropdown ${isMenuOpen?"show":""}`}>
                        {allProjectsName.map((projectName) => (
                            <a href={`/project/${projectName.slug.current}`}>{projectName.title}</a>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="project-middle">
                        ...
                        <ul>
                            {project.technologies.map((technology, index) => (
                                <li key={index} className="technology-item">{technology}</li>
                            ))}
                        </ul>
                    </div>
                    ...
                    {isModalOpen && currentImageIndex !== null && (
                        <div className="modal" onClick={closeModal}>
                            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                                ...
                                <button className="prev-btn" onClick={handlePrevImage} aria-label="Previous Image">&#8249;</button>
                                <button className="next-btn" onClick={handleNextImage} aria-label="Next Image">&#8250;</button>
                            </div>
                        </div>
                    )}
                </div>
